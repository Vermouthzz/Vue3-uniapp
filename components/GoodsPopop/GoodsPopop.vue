					...
					<view class="popup-body flex-c">
						<block v-for="item in props.goods?.sku_list" :key="item.name">
							...
							<view class="body-item-block flex">
								<view class="body-item" v-for="(subItem,index) in item.value" :key="index" @tap="onSelectSku(subItem,item.value)" :class="{border_active: subItem.selected,disabled: subItem.disabled}">
									{{subItem.name}}
								</view>
							</view>
						</block>
