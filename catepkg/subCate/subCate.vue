			...
			<view class="submit-header-btm">
				<scroll-view class="scroll-x-block" scroll-x="true" :scroll-left="scrollX" scroll-with-animation="true">
					<view class="block" v-for="(item,index) in subcateNav" :key="item.id" @tap="onSwitchTab(item,index)">
						<view class="third-cate-title" :class="{border_btm: goodsId == item.category_id}">
							{{item.category_name}}
						</view>
					</view>
				</scroll-view>
				...
				<view class="scroll-y-block flex-c">
					<view class="second-cate-block flex-c">
						...
						<view class="third-cate-block flex">
							<view class="third-cate-item" v-for="(item) in subcateList" :key="item.goods_id">
...
const getSubCateList = async (parent_id, id) => {
	const res = await getSubcateListAPI(parent_id, id)
	subcateList.value = res.result.goodsData
	if(res.result.navList.length) subcateNav.value = res.result.navList
